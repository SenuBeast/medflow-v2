    ...
    return (
        <div className="space-y-4 md:space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div>
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <h1 className="text-lg md:text-2xl font-semibold text-text-main">Controlled Substances Registry</h1>
                    </div>
                    <p className="text-xs md:text-sm text-text-sub mt-2">
                ...
                <div className="w-full sm:w-auto">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 md:py-2 border border-border-main rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500"
                            value={searchTerm}
            ...
            <div className="md:hidden space-y-3">
                <div className="bg-red-50 px-4 py-2.5 rounded-xl border border-red-100 flex items-center gap-2">
                    <AlertCircle size={14} className="text-red-600 shrink-0" />
                    <span className="text-[10px] font-medium text-red-800 uppercase tracking-wider">DEA / Regulatory Compliance Active</span>
                </div>
                {controlledItems.length === 0 ? (
                    <div className="bg-card rounded-xl border border-red-100 p-8 text-center text-text-sub">
                    ...
                    controlledItems.map((item) => (
                        <div key={item.id} className="bg-card rounded-xl border border-red-100 p-4 space-y-3">
                            <div className="flex items-start justify-between gap-2">
                                <div>
                                    <p className="text-sm font-medium text-text-main">{item.name}</p>
                                    <p className="text-[10px] text-text-sub mt-0.5">SKU: {item.sku}</p>
                                </div>
                                {item.quantity <= item.minimum_order_quantity && (
                                    <span className="text-[10px] text-red-600 font-bold uppercase bg-red-50 px-2 py-0.5 rounded-full border border-red-200 shrink-0">Low</span>
                                )}
                            </div>
                            <div className="grid grid-cols-3 gap-2 text-xs">
                                <div>
                                    <span className="text-text-dim">Category</span>
                                    <p className="font-medium text-text-main truncate">{item.category}</p>
                                </div>
                                <div>
                                    <span className="text-text-dim">Stock</span>
                                    <p className={`font-semibold tabular-nums ${item.quantity <= item.minimum_order_quantity ? 'text-red-600' : 'text-text-main'}`}>
                                        {item.quantity} {item.unit}
                                    </p>
                                </div>
                                <div>
                                    <span className="text-text-dim">Expiry</span>
                                    <p className={`font-medium ${item.batches?.[0]?.expiry_date && new Date(item.batches[0].expiry_date) < new Date() ? 'text-red-600' : 'text-text-main'}`}>
                                        {item.batches?.[0]?.expiry_date ? format(new Date(item.batches[0].expiry_date), 'MMM d, yyyy') : '-'}
                                    </p>
                                </div>
                            </div>
                            {canManage ? (
                                <button
                                    ...
                                    className="w-full inline-flex items-center justify-center gap-2 px-3 py-2.5 bg-red-100 text-red-700 hover:bg-red-200 rounded-lg text-xs font-medium transition-colors border border-red-200 min-h-[40px]"
                                >
                                    <Fingerprint size={14} />
                                    Log Dispensation
                                </button>
                            ) : (
                                <p className="text-xs text-text-dim italic text-center">View Only</p>
                            )}
                        </div>
                    ))
                )}
            ...
            {/* Desktop Table View */}
            <div className="hidden md:block bg-card rounded-xl shadow-sm border border-red-100 overflow-hidden">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-surface-dim border-b border-border-dim">
                                <th className="px-4 lg:px-6 py-4 text-xs font-semibold text-text-sub tracking-wider">Item Details</th>
                                <th className="px-4 lg:px-6 py-4 text-xs font-semibold text-text-sub tracking-wider">Category</th>
                                <th className="px-4 lg:px-6 py-4 text-xs font-semibold text-text-sub tracking-wider text-right">Current Stock</th>
                                <th className="px-4 lg:px-6 py-4 text-xs font-semibold text-text-sub tracking-wider">Expiry Warning</th>
                                <th className="px-4 lg:px-6 py-4 text-xs font-semibold text-text-sub tracking-wider text-right">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {controlledItems.length === 0 ? (
                                ...
                                controlledItems.map((item) => (
                                    <tr key={item.id} className="hover:bg-red-50/10 transition-colors">
                                        <td className="px-4 lg:px-6 py-4 border-b border-gray-50">
                                            <div>
                                                <p className="text-sm font-medium text-text-main">{item.name}</p>
                                                <p className="text-xs text-text-sub mt-0.5">SKU: {item.sku}</p>
                                            </div>
                                        </td>
                                        <td className="px-4 lg:px-6 py-4 text-sm text-text-sub border-b border-gray-50">
                                            {item.category}
                                        </td>
                                        <td className="px-4 lg:px-6 py-4 border-b border-gray-50 text-right">
