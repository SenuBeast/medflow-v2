...
export function ControlledSubstancesPage() {
    ...
    if (isLoading) return <div className="p-8 text-center text-gray-500">Loading controlled register...</div>;
            ...
            <div className="flex justify-between items-start">
                <div>
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl font-semibold text-gray-900">Controlled Substances Registry</h1>
                    </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-100">
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 tracking-wider">Item Details</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 tracking-wider">Category</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 tracking-wider text-right">Current Stock</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 tracking-wider">Expiry Warning</th>
                            <th className="px-6 py-4 text-xs font-semibold text-gray-500 tracking-wider text-right">Actions</th>
                        </tr>
                    </thead>
                                ...
                                <tr key={item.id} className="hover:bg-red-50/10 transition-colors">
                                    ...
                                    <td className="px-6 py-4 border-b border-gray-50 text-right">
                                        <div className="flex flex-col items-end">
                                            <span className={`text-sm font-semibold tabular-nums ${item.quantity <= item.minimum_order_quantity ? 'text-red-600' : 'text-gray-900'}`}>
                                    ...
                                    <td className="px-6 py-4 border-b border-gray-50">
                                        {item.batches?.[0]?.expiry_date ? (
                                            <span className={`text-sm ${new Date(item.batches[0].expiry_date) < new Date() ? 'text-red-600 font-semibold' : 'text-gray-600'}`}>
