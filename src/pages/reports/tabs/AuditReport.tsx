...
function AuditTable({ filters }: AuditReportProps) {
    ...
    const actionColor = (action: string) => {
        ...
        return 'bg-gray-100 text-text-sub';
    };
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {isLoading ? (
                                ...
                                data.map(log => (
                                    <tr key={log.id} className="hover:bg-surface-dim/60 transition-colors">
                                        ...
                                        <td className="px-5 py-3.5 text-text-sub capitalize">{log.entity_type}</td>
                                        <td className="px-5 py-3.5 font-mono text-xs text-text-dim max-w-[120px] truncate">{log.entity_id ?? '—'}</td>
                                        <td className="px-5 py-3.5 text-text-sub whitespace-nowrap text-xs">
            ...
            fallback={
                <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-text-dim text-sm mt-1">You need <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">admin.audit.view</code> permission to view audit logs.</p>
                    </div>
                </div>
            }
        >
