...
async function fetchUserProfile(userId: string): Promise<User | null> {
    ...
    if (userError) {
        console.error('[DEBUG] Supabase users table error:', userError);
    }
...
async function ensureUserRecord(userId: string, email: string): Promise<void> {
    ...
    if (selectError) {
        console.error('[DEBUG] users select error:', selectError);
    }
    ...
    if (roleError || !viewerRole) {
        console.error('[DEBUG] Failed to fetch Viewer role:', roleError || 'Role not found');
        return;
    }
    ...
    if (viewerRole) {
        ...
        if (insertError) {
            console.error('[DEBUG] Failed to insert new user:', insertError);
        } else {
            console.log('[DEBUG] Successfully created missing user profile for:', email);
        }
    }
}
